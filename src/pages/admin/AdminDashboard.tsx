...
import { Newspaper, CalendarDays, Image, Heart } from "lucide-react";
import { Link } from "react-router-dom";

const AdminDashboard = () => {
  const [counts, setCounts] = useState({ news: 0, events: 0, gallery: 0, sponsors: 0 });

  useEffect(() => {
    const fetchCounts = async () => {
      const [news, events, gallery, sponsors] = await Promise.all([
