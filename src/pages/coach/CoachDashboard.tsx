...
import { Users, ClipboardList, Mail, Clock } from "lucide-react";
...
const CoachDashboard = () => {
  const { user } = useAuth();
  const [stats, setStats] = useState<Stats>({ players: 0, teams: 0, emails: 0 });
  ...
  useEffect(() => {
    ...
    const load = async () => {
      const [playersRes, teamsRes, emailsRes, profileRes] = await Promise.all([
